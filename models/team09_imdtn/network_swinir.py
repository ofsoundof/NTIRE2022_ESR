...
class RSTB(nn.Module):
    ...
    def __init__(self, dim=64, input_resolution=[48,48], depth=2, num_heads=2, window_size=6,
                 ...
                 drop_path=0., norm_layer=nn.LayerNorm, downsample=None, use_checkpoint=False,
                 img_size=48, patch_size=4,upscale=2):
        ...
        if downsample is not None:
            ...
            self.upsampler = upsample_block(dim*2, dim, upscale_factor=upscale)
        else:
            self.downsample = None
    ...
    def forward(self, x, x_size):
        ...
        out = self.residual_group(x_embed, x_size)
        if self.downsample is not None:
...
class PatchUnEmbed(nn.Module):
    ...
    def __init__(self, img_size=48, patch_size=4, in_chans=64, embed_dim=96, norm_layer=None):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        patches_resolution = [img_size[0] // patch_size[0], img_size[1] // patch_size[1]]
