...
class RFDB(nn.Module):
    ...
    def switch_to_deploy(self):
        for conv in [self.c1_r, self.c2_r, self.c3_r]:
            kernel_value = np.zeros((conv.out_channels, conv.in_channels, 3, 3), dtype=np.float32)
            for i in range(conv.out_channels):
                kernel_value[i, i, 1, 1] = 1
            kernel_value = torch.from_numpy(kernel_value).cuda()
            conv.weight.data = conv.weight.data + kernel_value
...
class RFDN(nn.Module):
    def __init__(self, conv=default_conv, in_nc=3, nf=50, num_modules=4, out_nc=3, upscale=4):
